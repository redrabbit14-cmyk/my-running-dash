...
def get_notion_data():
    ...
    for page in all_pages:
        ...
        try:
            ...
            # 사진 정보 가져오기 (텍스트 또는 URL 컬럼 기준)
            photo_prop = p.get("사진", {})
            photo = None
            if photo_prop.get("type") == "rich_text":
                texts = photo_prop.get("rich_text", [])
                if texts: photo = texts[0].get("text", {}).get("content", "")
            elif photo_prop.get("type") == "url":
                photo = photo_prop.get("url", "")
            ...
            if name and date_str:
                records.append({
                    ...
                    "elevation": elev,
                    "photo": convert_google_drive_link(photo)
                })
        except: continue
