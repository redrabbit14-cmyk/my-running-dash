...
# 2. 모바일 최적화 CSS (날씨 관련 스타일 제거 및 경량화)
st.markdown("""
<style>
...
# 3. 데이터 로드 함수 (에러 해결 포인트: notion.databases.query 방식)
...
def get_ai_recommendation(crew_data):
    try:
        ...
        response = requests.post(
            ...
            json={'model': 'claude-3-5-sonnet-20240620', 'max_tokens': 500, 'messages': [{'role': 'user', 'content': prompt}]}
        )
        return response.json()['content'][0]['text'] if response.ok else "AI 조언을 가져올 수 없습니다."
    except: return "추천 생성 오류 발생"
